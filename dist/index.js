...
export class DealProcessor {
     ...
     */
    async scanPipelineForPendingDeals() {
        const pendingDeals = [];
        try {
     ...
     */
    getSpecificationForStage(stage) {
        return this.stageMapping[stage] || null;
    }
    /**
     * Load deal data from JSON file
     */
    async loadDeal(dealPath) {
     ...
     */
    async runAnalysisSpec(specFile, deal, dealInfo) {
     ...
     */
    async analyzeUsingSpec(spec, deal, dealInfo) {
        ...
        const analysis = {
            dealId: deal.id,
            stage: stageNumber,
            stageName: this.stageNames[dealInfo.stage] || dealInfo.stage,
            ...
            keyFindings: [
                ...
                `Current Stage: ${this.stageNames[dealInfo.stage]}`
            ],
     ...
     */
    determineRecommendation(deal, stage) {
        // Basic screening logic that varies by stage
        switch (stage) {
            case 'A-initial-intake':
                if (!deal.lihtcInfo.currentlyLIHTC)
                    return 'REJECT';
                if (deal.basicInfo.pricePerUnit > 200000)
                    return 'REJECT';
                if (deal.basicInfo.pricePerUnit < 30000)
                    return 'REJECT';
                if (deal.financialData.occupancyRate < 80)
                    return 'REQUEST_MORE_INFO';
                return 'ADVANCE';
            case 'B-preliminary-analysis':
                if (deal.basicInfo.pricePerUnit > 150000)
                    return 'REJECT';
                if (deal.financialData.expenseRatio > 50)
                    return 'REQUEST_MORE_INFO';
     ...
     * Move deal based on analysis result
     */
    async moveDealBasedOnResult(dealInfo, analysis) {
     ...
     */
    async saveAnalysis(dealPath, specFile, analysis) {
