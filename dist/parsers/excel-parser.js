...
export class ExcelParser {
     ...
     */
    async parseFinancials(filePath) {
        try {
            console.log(`Parsing financials: ${filePath}`);
            const workbook = XLSX.readFile(filePath);
            // Look for financial data in multiple possible sheets
            const potentialSheets = workbook.SheetNames.filter(name => name.toLowerCase().includes('income') ||
    ...
    findHeaderRow(data) {
        ...
        for (let i = 0; i < Math.min(10, data.length); i++) {
            ...
            const matchCount = indicators.filter(indicator => rowStr.includes(indicator)).length;
            if (matchCount >= 2) {
                return i;
            }
        }
    ...
    parseRentRollUnits(headers, dataRows) {
        const units = [];
    ...
    parseRentRollRow(headerMap, row) {
        try {
    ...
    calculateRentRollSummary(units) {
