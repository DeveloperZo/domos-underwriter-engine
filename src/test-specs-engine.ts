...
async function testSpecsEngine() {
  ...
  try {
    ...
    const stage1Analysis = await specsEngine.analyzeWithSpec(
      'A-initial-intake',
      dealData,
      tenantData,
      financialData
    ...
    if (stage1Analysis.recommendation === 'ADVANCE') {
      ...
      const stage2Analysis = await specsEngine.analyzeWithSpec(
      ...
      if (stage2Analysis.recommendation === 'ADVANCE') {
        ...
        const stage3Analysis = await specsEngine.analyzeWithSpec(
          'C-full-underwriting',
          dealData,
          tenantData,
          financialData
